...
//Define this constant if you want to enter debugging mode
//define("DEBUG", 'Debug Mode is On');
 
 


//define("URL", 'http://api06.dev.openstreetmap.org/api/0.6/');
define("URL", 'http://api.openstreetmap.org/api/0.6/');
//define("URL", 'http://api06.openstreetmap.org/api/0.6/');
define("USERAGENT", 'AppEngine');
...
class osm {
	...
	private function curl($url, $data) {
		...
		{
			print "<br />s = " . $s;
			print "<br /> URL = " . URL.$url;
			print "<br /> USERAGENT = " . USERAGENT;
			if ($result) 
				print "<br />result=" . $result; 
			else 
				print "<br /> yi sasura kaam naa kari";
			print "<br /> self:errno = ". self::$errno;
		}
		//if($result==0)
	...
	private function getObject($type, $id, $extra = null) {
		// bypass integer check... if (!is_integer($id) || $id < 1) {
		if ($id < 1) {
			self::$errno = 500;
			//sazal
			if(defined('DEBUG'))
			{
			print "<br />Error: " . self::$errno . "<br />";
			}
			//sazal
			return;
		}
		...
		//sazal
		if (defined('DEBUG')) echo "<br />url_to_curl() = " . $url;
		//sazal
...
class changeset {
	...
	private function curl($url, $data, $putpost) {
		...
		if(defined('DEBUG'))
		{
			print "<br/> <br /> From the middle of hell<br />". self::$errno = curl_getinfo($s,CURLINFO_HTTP_CODE);
			print "tempnam = " . $tmpfname;
		}
	...
	private function modifyObject($type, $id, $document) {
		if (self::$changesetid > 0 && self::$atomic === false) {
			$url = $type.'/'.$id;
			return self::curl($url, '<osm>'.$document.'</osm>', true);
		} else 	if (self::$atomic === true) {
...
if(defined('DEBUG'))print('<br/>reached at the bottom of the page');
?>